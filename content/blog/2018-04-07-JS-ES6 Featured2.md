...
#### 8. Template Literals
- ES6부터는 Template Literals은 직관성을 포함하여, 연산까지 `${}`에서 가능하기때문에 많은 가능성을 열어주었습니다.

```js
let customer = { name: "Seolhun" }
let card = { amount: 10, product: "Computer", unitprice: 42 }
let message = `Hello ${customer.name}, want to buy ${card.amount} ${card.product} for a total of ${card.amount * card.unitprice} bucks?`
// "Hello Seolhun, want to buy 10 Computer for a total of 420 bucks?"
```
