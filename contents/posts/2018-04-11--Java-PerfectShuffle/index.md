...
## 2. Process
1. T 입력 값을 통해 문제 개수만큼 루프문을 만든다.
2. 문자의 수 만큼 루프문을 통해 입력 값을 받는다.
3. 문제 수가 짝수면 똑같이, 홀수면 앞에 카드덱에 1장을 더 넣는다.
4. 나누어진 배열 순서에 맞게 셔플한다.
5. 출력한다.
...
## 4. Outro
기본적인 자료구조를 많이 이용했습니다. Data의 개수가 서로 다르기 때문에 List로 개수만큼 파악해야하는 것은 불안정성이 크다고 생각하여 Queue로 대체하였습니다. 추가적인 사항들은 더 학습하여 정리하여 올리겠습니다.

다음에는 레벨 4에 쉬운 문제를 풀어보겠습니다.
